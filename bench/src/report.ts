...
const renderTakeaways = (report: BenchReport): string => {
  ...
  lines.push('  • Timed library scenarios use `nullable: false` on non-null sort keys.')
  lines.push('    Emission is dialect-specific: Postgres/SQLite → row compare; MSSQL → plain OR;')
  lines.push('    MySQL stays on null-safe OR (best seek form on that engine). See plans (PG).')
  lines.push('  • Default (unmarked) leading sorts stay null-safe everywhere — EXPLAIN-only on')
  lines.push('    Postgres (often ≈ OFFSET). Mark NOT NULL keys with `nullable: false` in apps.')
  lines.push('  • ideal-baseline is raw SQL matching the dialect’s library emission (no codec).')
  lines.push('    Library deep-page should approach it; large gaps usually mean plan mismatch.')
