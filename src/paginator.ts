...
}: PaginateArgs<DB, TB, O, S> & PaginatorOptions): Promise<
  PaginatedResult<O>
> => {
  ...
  try {
    const decodedCursor = cursor
      ? await decodeCursor(cursor, cursorCodec)
      : null;
    const sortsApplied =
      decodedCursor?.type === "prev" ? invertSorts(sorts) : sorts;
    ...
    if (decodedCursor) {
      if (decodedCursor.type === "offset") {
        ...
        if (decodedCursor.payload.sig !== sig)
          throw new PaginationError("Page token does not match sort order");
    ...
    const items =
      decodedCursor?.type === "prev"
        ? rows.slice(0, limit).reverse()
        : rows.slice(0, limit);

    const { startCursor, endCursor, prevPage, nextPage } =
      await resolvePageTokens(
        items,
        sorts,
        cursorCodec,
        decodedCursor,
...
const assertLimitSorts = (limit: number, sorts: unknown[]) => {
  if (!(Number.isInteger(limit) && limit > 0))
    throw new PaginationError("Invalid page size limit");
  if (!Array.isArray(sorts) || sorts.length < 1)
    throw new PaginationError("Cannot paginate without sorting");
};
