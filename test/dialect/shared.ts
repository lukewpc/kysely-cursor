...
export const createTestHelpers = (
  ...
  const baseBuilder = () =>
    db
      .selectFrom("users")
      .select(["id", "name", "created_at", "rating", "active"]);

  const fetchAllPlainSorted = async (
    sorts: SortSet<TestDB, "users", TestRow>,
  ) => {
    let q = baseBuilder();
...
export const resolveNextPageToken = async (
  items: TestRow[],
  sorts: SortSet<TestDB, "users", TestRow>,
) => {
  const cursorCodec = codecPipe(superJsonCodec, base64UrlCodec);
  if (items.length === 0)
    throw new Error("Cannot build next page token from empty items");
...
export const runSharedTests = (
  ...
  it("throws on malformed page tokens", async () => {
    ...
    await expect(
      page(5, sorts, "this-is-not-a-valid-token"),
    ).rejects.toThrowError(/Failed to paginate/i);
  });
  ...
  it("throws when page token does not match the provided sort signature", async () => {
    ...
    await expect(page(3, sortsB, first.nextPage)).rejects.toThrowError(
      /Page token does not match sort order/i,
    );
  });
  ...
  it("orders boolean DESC with a clean true-prefix before falses", async () => {
    ...
    expect(
      first.items.slice(0, firstFalseIdx).every((r) => r.active === true),
    ).toBe(true);
    // everything after must be false (since we fetched all)
    expect(
      first.items.slice(firstFalseIdx).every((r) => r.active === false),
    ).toBe(true);
  });
  ...
  it("validates limit and sorts (throws on invalid limit)", async () => {
    ...
    // Invalid: limit <= 0
    await expect(page(0, sorts)).rejects.toThrowError(
      /Invalid page size limit/i,
    );
  });
  ...
  it("supports prevPage navigation (backward) and preserves item order", async () => {
    ...
    expect(forwardAgain.items.map((r) => r.id)).toEqual(
  ...
  it("starts mid-way with offset, then continues using cursor tokens", async () => {
    ...
    expect(mid.items.map((r) => r.id)).toEqual(
      expected.slice(5, 10).map((r) => r.id),
    );
    ...
    expect(seenForward.map((r) => r.id)).toEqual(
      expected.slice(5).map((r) => r.id),
    );
    ...
    expect(back.items.map((r) => r.id)).toEqual(
      expected.slice(0, 5).map((r) => r.id),
    );
  });
};
